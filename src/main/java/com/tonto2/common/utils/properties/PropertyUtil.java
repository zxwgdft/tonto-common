package com.tonto2.common.utils.properties;
	...
	public static Map<String, String> getProperties(String file) {
		...
		if (properties == null) {
			...
			try {
				...
				props.load(PropertyUtil.class.getClassLoader().getResourceAsStream(file));
				...
				logger.error("Can not read properties file :" + file, e);
				return null;
			}
		...
		if (cache == null) {
			...
			synchronized (ConfigCache.class) {
				...
				if (cache == null) {
					...
					URL url = PropertyUtil.class.getClassLoader().getResource(file);

					if (url == null) {
						logger.error("Can not find properties file :" + file);
						return null;
					}
					...
					try {
						...
						logger.error("Can not find properties file :" + file, e);
						return null;
					}
