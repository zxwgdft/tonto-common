...
public class JsonConvert {
	...
	/**
	 * 根据属性名称添加自定义的转化规则，同名属性都会被转化，需注意使用，
	 * 根据属性名称的自定义转化规则优先级最高
	 * @param name
	 * @param convert
	 */
	public void addConvertByName(String name,Convert<?> convert)
	{
		if(privateName2ConvertMap==null)
			privateName2ConvertMap=new HashMap<String,Convert<?>>();
		privateName2ConvertMap.put(name, convert);	
	}
	...
	public <T> List<T> convertList(List<?> list,Class<T> clazz,String name,FieldValidator validator) throws JsonValueValidateException, JsonConvertException
	{
		if(list==null)
			return null;
		
		Convert<?> convert=getConvert(name,clazz);
		
		List<ValidateResult> vresults=null;
		...
		for(Object obj:list)
		{
			...
			{
				try{
					result.add((T) convertMap((Map<String, Object>)obj,clazz,name,validator));
				}
				catch(JsonValueValidateException e)
				{
					if(vresults==null)
						vresults=new ArrayList<ValidateResult>();
					vresults.add(e.getErrorResult());
				}
			}			
			else
			{
		...
		if(vresults!=null)
			throw new JsonValueValidateException(new ValidateResult(false,name,"验证失败",vresults,true));
		return result;
	}
	
	@SuppressWarnings("unchecked")
	public <T> T[] convertArray(List<?> list,Class<T> clazz,String name,FieldValidator validator) throws JsonValueValidateException, JsonConvertException
	{
		if(list==null)
			return null;
		...
		for(Object obj:list)
		{
			...
			{
				try{
					result[i]=(T) convertMap((Map<String, Object>)obj,clazz,name,validator);
				}
				catch(JsonValueValidateException e)
				{
					if(vresults==null)
	...
	public <T> T convertMap(Map<String,Object> map,Class<T> clazz,String fieldname,FieldValidator validator) throws JsonValueValidateException, JsonConvertException
	{
		if(clazz==null)
			return null;
					...
					try {
						...
						if(value instanceof List)
						{
							if(fieldClass.isArray())
							{
								...
								try{
									setValue=convertArray((List)value,arrayType,key,validator);
								}
								catch(JsonValueValidateException e)
								{
									validateResultList.add(e.getErrorResult());
									break;
								}
							}	
							else if(List.class.isAssignableFrom(fieldClass))
							{
								...
								try{
									setValue=convertList((List)value,listType,key,validator);
								}
								catch(JsonValueValidateException e)
								{
									validateResultList.add(e.getErrorResult());
						...
						else if(value instanceof Map)
						{
							...
							try{
								setValue=convertMap(_map,fieldClass,key,validator);
							}
							catch(JsonValueValidateException e)
							{
								validateResultList.add(e.getErrorResult());
						...
						{
							Convert<?> convert=getConvert(key,fieldClass);
						...
						ValidateResult validateResult=validator.validate(field, setValue);
						if(!validateResult.isResult())
						{
							if(validateResultList==null)
								validateResultList=new ArrayList<ValidateResult>();
							validateResultList.add(validateResult);
							if(logger.isDebugEnabled())
								logger.debug("验证失败！"+validateResult.getMessage());
							break;
						}	
		...
		if(validateResultList.size()>0)
			throw new JsonValueValidateException(new ValidateResult(false,fieldname,"验证失败",validateResultList));
		
		return object;
	}
	
	
	private Convert<?> getConvert(String name,Class<?> clazz)
	{
		Convert<?> convert=null;
		
		if(name!=null&&privateName2ConvertMap!=null)	
			convert=privateName2ConvertMap.get(name);
